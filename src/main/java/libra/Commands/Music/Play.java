...
public class Play implements Command {
    ...
    public void run(SlashCommandEvent context, Document Guild, Config config) {
        ...
        if(raw == null) {
            context.reply(config.Emojis.Error+ " No has especificado ninguna canción").setEphemeral(true).queue();
            return;
        }
        ...
        if(Member != null && !Member.getVoiceState().inVoiceChannel()) {
            context.reply(config.Emojis.Error+ " Debes de estar en un canal de voz!").setEphemeral(true).queue();
            return;
        }
        ...
        if(context.getGuild().getSelfMember().getVoiceState().inVoiceChannel()){
            if(context.getGuild().getSelfMember().getVoiceState().getChannel() != VoiceChannel) {
                context.reply(config.Emojis.Error+ " Debes de estar en el mismo canal de voz que yo!").setEphemeral(true).queue();
            }
        }else {
